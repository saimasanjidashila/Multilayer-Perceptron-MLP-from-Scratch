...
class Softmax(ActivationFunction):
    ...
    def derivative(self, x: np.ndarray) -> np.ndarray:
        ...
        return softmax_x * (1 - softmax_x)  # Only useful for cross-entropy
...
class CrossEntropy:
    ...
    def derivative(self, y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
        return (y_pred - y_true) / y_true.shape[0]  # Proper gradient scaling
...
class MultilayerPerceptron:
    ...
    def train(self, x_train, y_train, x_val, y_val, learning_rate=0.001, batch_size=64, epochs=20):
       ...
       return training_losses, validation_losses  # ✅ Return the loss history
